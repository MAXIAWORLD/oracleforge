"""MAXIA Oracle — Chainlink on-chain price reader (Base mainnet).

Reads Chainlink Aggregator V3 contracts directly on Base mainnet via eth_call.
No centralized API — pure on-chain read (RPC -> smart contract -> price).

Extracted from MAXIA V12/backend/trading/chainlink_oracle.py on 2026-04-14.
No surgery applied: the V12 module has zero internal MAXIA dependencies, so
it was copied verbatim. Only this header was added.

Usage:
    price = await get_chainlink_price("ETH")
    # {"price": 3200.12, "decimals": 8, "round_id": 123, "updated_at": 1774...,
    #  "age_s": 5, "source": "chainlink_base"}
