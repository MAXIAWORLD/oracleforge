...
function getPeriodRange(period: Exclude<Period, "custom">): {
  from: string | null;
  to: string | null;
} {
  const now = new Date();
  ...
  if (period === "today")
    return { from: now.toISOString().slice(0, 10), to: null };
  if (period === "7d") {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-1.5 flex-1">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-xs text-green-400 font-600">
              {breakdown.local_pct.toFixed(1)}%
            </span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
            ...
            <span className="font-mono text-xs text-[--amber] font-600">
              {breakdown.cloud_pct.toFixed(1)}%
            </span>
          </div>
            ...
            {entries.map((e) => (
              <div key={e.name} className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-[10px] text-[--muted-fg] capitalize">
                    {e.name}
                  </span>
                </div>
...
function StatCard({
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-[11px] uppercase tracking-widest font-600 text-[--muted-fg]">
          {label}
        </span>
        <div
      ...
      <div
        className="font-heading font-800 text-3xl tracking-tight"
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex flex-col items-end gap-2">
              <div className="flex items-center gap-1 bg-white/[0.04] border border-[--border] rounded-lg p-1">
                {(
                  ["all", "month", "7d", "today"] as Exclude<Period, "custom">[]
                ).map((p) => (
                  <button
                    key={p}
                    onClick={() => setPeriod(p)}
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 px-3 py-1 rounded-md text-xs font-500 transition-all",
                  )}
                  style={
                    period === "custom"
                      ? { background: "var(--amber)", color: "#070a0f" }
        ...
        {totalBudget > 0 && (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="font-heading font-700 text-base">
                  Global Budget Health
                </h2>
                <p className="text-xs text-[--muted-fg] mt-0.5">
                  Aggregated across all budgeted projects
                </p>
              </div>
              <span
                className={cn(
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-mono text-[10px] text-[--muted-fg] truncate">
                          {p.api_key.slice(0, 32)}…
                        </p>
                      </div>
                      {hasBudget ? (
                        ...
                        <span className="text-[11px] text-[--muted-fg] italic shrink-0 w-28">
                          No budget
                        </span>
                      )}
        ...
        {!loading && projects.length > 0 && (
          ...
          >
            <p className="text-[11px] text-[--muted-fg]">
              ...
              <code className="font-mono text-[--amber] bg-[--amber-dim] px-1.5 py-0.5 rounded">
                base_url
              </code>{" "}
              →{" "}
