...
function useUsage(projectId: number): DailySpend[] | null {
  ...
  useEffect(() => {
    fetch(`/api/portal/usage?project_id=${projectId}`, {
      credentials: "include",
    })
...
function UsageChart({ projectId }: { projectId: number }) {
  ...
  if (!daily)
    return (
      <div
        className="h-[120px] flex items-center justify-center text-xs"
        style={{ color: "#c8d8e8" }}
      >
        Loading…
      </div>
  ...
  return (
    <div className="mt-4">
      ...
      {hasData ? (
        ...
        <p className="text-xs text-center py-8" style={{ color: "#c8d8e8" }}>
...
function PortalContent() {
  ...
  async function handleRequest(e: FormEvent) {
    ...
    try {
      ...
      setError(
        "Could not send the link right now. Please check your connection and try again.",
      );
    } finally {
      setLoading(false);
    }
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 bg-black/30 rounded-md px-3 py-2">
                    ...
                    <CopyButton
                      text={`${API_BASE}/proxy/ollama/v1/chat/completions`}
                    />
                  </div>
                </div>
              </div>
              ...
              <p className="text-[10px] mt-3" style={{ color: "#c8d8e8" }}>
                ...
                <code className="text-[--amber]">
                  X-Provider-Key: your-openai-key
                </code>{" "}
                to each request. Ollama routes require no provider key — local
                inference is always free.
              </p>
            </div>
          ))}
        </div>
      ...
      {sent ? (
        ...
        >
          ...
          <p className="text-sm" style={{ color: "#c8d8e8" }}>
            A magic link has been sent to <strong>{email}</strong>. It expires
            in 1 hour.
          </p>
        </div>
      ) : (
