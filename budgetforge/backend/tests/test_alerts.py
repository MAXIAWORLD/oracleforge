...
class TestAlertTriggered:
    ...
    async def test_alert_sent_when_threshold_crossed(self, client):
        ...
        proj = (
            await client.post(
                "/api/projects",
                json={"name": "alert-test", "alert_email": "user@test.com"},
            )
        ).json()
        await client.put(
        ...
        with (
            ...
            patch(
                "services.proxy_dispatcher.maybe_send_alert", new_callable=AsyncMock
            ) as mock_alert,
        ):
    ...
    async def test_alert_not_sent_below_threshold(self, client):
        ...
        proj = (
            await client.post(
                "/api/projects",
                json={"name": "no-alert-test", "alert_email": "user@test.com"},
            )
        ).json()
        await client.put(
            f"/api/projects/{proj['id']}/budget",
    ...
    async def test_alert_sent_only_once_per_threshold_crossing(self, client):
        ...
        proj = (
            await client.post(
                "/api/projects",
                json={"name": "dedup-test", "alert_email": "user@test.com"},
            )
        ).json()
        await client.put(
            f"/api/projects/{proj['id']}/budget",
        ...
        with (
            ...
            # 2e appel → seuil déjà franchi, alert déjà envoyée pour cette période
            await client.post(
                "/proxy/openai/v1/chat/completions",
                json={
                    "model": "gpt-4o",
    ...
    async def test_no_alert_without_alert_email(self, client):
        ...
        proj = (
            await client.post("/api/projects", json={"name": "no-email-test"})
        ).json()
        await client.put(
            f"/api/projects/{proj['id']}/budget",
    ...
    async def test_alert_email_persisted_on_project(self, client):
        ...
        proj = (
            await client.post(
                "/api/projects",
                json={"name": "email-persist", "alert_email": "alex@test.com"},
            )
        ).json()
    ...
    async def test_alert_retried_when_email_fails(self, client):
        ...
        proj = (
            await client.post(
                "/api/projects",
                json={"name": "email-fail-retry", "alert_email": "user@test.com"},
            )
        ).json()
        await client.put(
            f"/api/projects/{proj['id']}/budget",
