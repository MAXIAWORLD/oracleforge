...
class TestSsrfDnsRebinding:
    ...
    def test_ssrf_unresolvable_domain_is_blocked(self):
        ...
        with patch(
            "core.url_validator.socket.getaddrinfo", side_effect=OSError("NXDOMAIN")
        ):
...
class TestAppUrlStartupWarning:
    @pytest.mark.asyncio
    async def test_startup_logs_warning_if_app_url_not_https_in_production(
        self, caplog
    ):
        ...
        assert any(
            "http" in r.message.lower() or "app_url" in r.message.lower()
    ...
    async def test_startup_no_warning_when_app_url_is_https(self, caplog):
        ...
        app_url_warnings = [
            r
            for r in caplog.records
            if "app_url" in r.message.lower()
            or ("http" in r.message.lower() and "https" not in r.message.lower())
        ]
