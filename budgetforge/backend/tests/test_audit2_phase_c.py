...
class TestStreamingCancelsUsageOnError:
    ...
    async def test_stream_error_cancels_prebilled_usage(self, client, test_db):
        ...
        with patch(
            "routes.proxy.ProxyForwarder.forward_openai_stream",
    ...
    async def test_stream_success_keeps_usage(self, client, test_db):
        ...
        with patch(
            "routes.proxy.ProxyForwarder.forward_openai_stream", side_effect=ok_stream
        ):
            resp = await client.post(
                "/proxy/openai/v1/chat/completions",
                headers={"Authorization": f"Bearer {proj.api_key}"},
...
class TestProxyRetries:
    ...
    async def test_proxy_retries_on_5xx_error(self, client, test_db):
        ...
        with patch(
            "routes.proxy.ProxyForwarder.forward_openai", side_effect=flaky_forward
        ):
            resp = await client.post(
                "/proxy/openai/v1/chat/completions",
                headers={"Authorization": f"Bearer {proj.api_key}"},
        ...
        assert call_count == 2, (
            f"forward_openai devrait être appelé 2x, appelé {call_count}x."
        )
    ...
    async def test_proxy_retries_exhausted_returns_502(self, client, test_db):
        ...
        with patch(
            "routes.proxy.ProxyForwarder.forward_openai", side_effect=always_fail
        ):
            resp = await client.post(
                "/proxy/openai/v1/chat/completions",
                headers={"Authorization": f"Bearer {proj.api_key}"},
    ...
    async def test_proxy_no_retry_on_4xx(self, client, test_db):
        ...
        with patch(
            "routes.proxy.ProxyForwarder.forward_openai", side_effect=auth_error
        ):
            await client.post(
                "/proxy/openai/v1/chat/completions",
                headers={"Authorization": f"Bearer {proj.api_key}"},
